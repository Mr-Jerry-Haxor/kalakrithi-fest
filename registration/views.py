...
def register(request):
  if request.user.is_authenticated:
    if request.method == 'GET':
      ...
      return render(request, 'register.html', {'form': form, 'passessold':passessold})
...
def verify(request):
  if request.user.is_authenticated:
    if request.method == "GET":
      ...
      return render(request, 'verify.html', {'form': form, 'admitted': admitted})
    else:
      qrcode = request.POST.get('qrtext')
      utc = datetime.now()
      ...
      if verification_status.objects.filter(qrhash=qrcode).exists():
        return render(request, 'message.html', {'message': 'The participant is already Admitted'})
      else:
        vdata = {
...
def verifydetails(request):
  if request.user.is_authenticated:
    if request.method == 'POST':
      ...
      status = "IN"
      status_update = verification_status(fullname=fullname,
                                          registrationnumber = registrationnumber,
                                          email = email,
                                          qrtext = qrtext,
                                          qrhash =qrhash,
